...
class ProxyEncoderBlock(nn.Module):
    ...
    def __init__(
        ...
        # TODO: parametrize teacher hidden dim
    ...
    def forward(self, input: torch.Tensor, memory: torch.Tensor):
        """ Forward pass for the proxy encoder block.
        :param input: input tensor of shape (batch_size, seq_length, hidden_dim)
        :param memory: memory tensor of shape (batch_size, seq_length, teacher_hidden_dim)
        """

        torch._assert(input.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {input.shape}")
        x = input
        x = self.ln_1(x)
        x, _ = self.cross_attention(x, memory, memory, need_weights=False)
        x = self.dropout(x)
        x = x + input
        # TODO: add self-attention before or after Cross-Attention?
        y = self.ln_2(x)
        y = self.mlp(y)
        return x + y
...
class VisionTransformer(BaseModel):
    ...
    def forward(self, x: torch.Tensor, memory: Optional[torch.Tensor] = None):
        ...
        x = self.encoder(x, memory=memory)
